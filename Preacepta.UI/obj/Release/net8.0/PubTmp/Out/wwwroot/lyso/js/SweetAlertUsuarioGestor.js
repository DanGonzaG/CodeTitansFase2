document.addEventListener('DOMContentLoaded', function () {
    ...
    /*SweetAler, se encarga de buscar ABOGADO y valida su existencia en el sistema*/
    document.getElementById('BuscarDocumento').addEventListener('click', function (event) {
        event.preventDefault();
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('/HistorialDocumentos/IdExiste?id=' + result.value)
                    .then(respuesta => respuesta.json())
                    .then(datos => {
                        if (datos.bandera) {
                            window.location.href = '/HistorialDocumentos/Details?id=' + result.value;
                        }
                        else {
                            Swal.fire('Sistema de busqueda', `No se halló coincidencia para el documento número ${result.value}.`, 'error')
                        }
                    });
            }
        });
