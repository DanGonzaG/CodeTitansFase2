...
{
    public class RegisterClienteGestorModel : PageModel
    {
        private readonly SignInManager<IdentityUser> _signInManager;
        private readonly UserManager<IdentityUser> _userManager;
        private readonly IUserStore<IdentityUser> _userStore;
        private readonly IUserEmailStore<IdentityUser> _emailStore;
        private readonly ILogger<RegisterClienteGestorModel> _logger;
        ...
        public RegisterClienteGestorModel(
            UserManager<IdentityUser> userManager,
            IUserStore<IdentityUser> userStore,
            SignInManager<IdentityUser> signInManager,
            ILogger<RegisterClienteGestorModel> logger,
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                personaDTO.NumCedula = personaDTO.NumCedula.LimpiarCedula();
                
                while (await _buscarPersona.buscarXnumCedulaBOOLEAN(personaDTO.NumCedula) == true) 
                {
                    if (personaDTO.TipoIdentificacion.Equals("SinDocumento")) 
                    {
                        personaDTO.NumCedula.Equals("Sin número de identificación");
                        personaDTO.NumCedula = personaDTO.NumCedula.LimpiarCedula();
                    }
                    break;                    
                }
                ...
                var telefono1 = await _buscarPersona.buscarXtelefono1(personaDTO.Telefono1);
                ...
                int bandera =  await _crearPersonaLN.crear(personaDTO);//llamado de los LN y AD para crear la persona               
                #endregion
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(personaDTO.Email, "Confirma tu correo electrónico",
                        $"Por favor, confirme su cuenta mediante <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>haciendo clic aquí</a>.");

                    if (_userManager.Options.SignIn.RequireConfirmedAccount)
                    {
                        return RedirectToPage("RegisterConfirmation", new { email = personaDTO.Email, returnUrl = returnUrl });
