...
namespace Preacepta.UI.Controllers
{
    public class THistorialDocumento1Controller : Controller
    {
        ...
        {
            ViewData["TipoDocumento"] = new SelectList(_context.THistorialDocumentos, "Id", "TipoDocumento");
        ...
        public async Task<IActionResult> EditarDesdeHistorial(int id)
        {
            ...
            // Si ningún case hace un return
            return BadRequest("No se pudo determinar la acción a realizar.");
        }
        ...
        public async Task<IActionResult> PrevisualizarDesdeHistorial(int Id)
        {
            ...
            switch (tipoDoc)
            {
                ...
                case "Compra y venta de vehículos":
                    {
                        var resultadoCVvehiculo = await _buscarcompraventaV.buscar(resultadoHistorial.IdDocumento);
                        if (resultadoCVvehiculo == null) return NotFound();
                        ...
                        string logoBase64CV = "";
                        var logoPath6 = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "lyso", "img", "PreaceptaLogoColorNegro.png");
                        if (System.IO.File.Exists(logoPath6))
                            logoBase64CV = Convert.ToBase64String(System.IO.File.ReadAllBytes(logoPath6));

                        int cedAbogado = resultadoCVvehiculo.CedulaAbogado;
                        int cedProp = resultadoCVvehiculo.CedulaPropietario;
                        int cedComp = resultadoCVvehiculo.CedulaComprador;

                        int idMarcaVeh = resultadoCVvehiculo.MarcaVehiculo;
                        int idTipoVeh = resultadoCVvehiculo.TipoVehiculo;
                        int idMarcaMotor = resultadoCVvehiculo.MarcaMotor;
                        int idComb = resultadoCVvehiculo.Combustible;

                        string placaVehiculo = resultadoCVvehiculo.PlacaVehiculo ?? "";
                        string modeloVehiculo = resultadoCVvehiculo.ModeloVehiculo ?? "";
                        string carroceria = resultadoCVvehiculo.Carroceria ?? "";
                        string categoria = resultadoCVvehiculo.Categoria ?? "";
                        string numeroMotor = resultadoCVvehiculo.NumeroMotor ?? "";
                        string chasis = resultadoCVvehiculo.Chasis ?? "";
                        string serie = resultadoCVvehiculo.Serie ?? "";
                        string vin = resultadoCVvehiculo.Vin ?? "";
                        string color = resultadoCVvehiculo.Color ?? "";
                        string anio = resultadoCVvehiculo.Anio.ToString() ?? "";
                        string capacidad = resultadoCVvehiculo.Capacidad?.ToString() ?? "";
                        string cilindraje = resultadoCVvehiculo.Cilindraje ?? "";
                        string precio = resultadoCVvehiculo.Precio.ToString() ?? "";
                        string monedaPrecio = resultadoCVvehiculo.MonedaPrecio ?? "";
                        string plazoOpcionAnios = resultadoCVvehiculo.PlazoOpcionAnios.ToString() ?? "";
                        string montoSenal = resultadoCVvehiculo.MontoSenal.ToString() ?? "";
                        string monedaSenal = resultadoCVvehiculo.MonedaSenal ?? "";
                        string montoADevolver = resultadoCVvehiculo.MontoADevolver.ToString() ?? "";
                        string montoAPerder = resultadoCVvehiculo.MontoAPerder.ToString() ?? "";
                        string monedaMontoPerd = resultadoCVvehiculo.MonedaMontoPerdido ?? "";

                        string fechaInicioMostrar =
                            DateTime.TryParse(resultadoCVvehiculo.FechaInicio, out var fi)
                                ? fi.ToString("dd/MM/yyyy")
                                : "";

                        string fechaFirmaMostrar =
                            DateTime.TryParse(resultadoCVvehiculo.FechaFirma, out var ff)
                                ? ff.ToString("dd/MM/yyyy")
                                : DateTime.Now.ToString("dd/MM/yyyy");

                        string horaFirmaMostrar = !string.IsNullOrWhiteSpace(resultadoCVvehiculo.HoraFirma)
                            ? (DateTime.TryParse(resultadoCVvehiculo.HoraFirma, out var dtHora) ? dtHora.ToString("hh:mm tt", CultureInfo.InvariantCulture)
                               : (TimeSpan.TryParse(resultadoCVvehiculo.HoraFirma, out var tsHora) ? DateTime.Today.Add(tsHora).ToString("hh:mm tt", CultureInfo.InvariantCulture)
                               : resultadoCVvehiculo.HoraFirma))
                            : DateTime.Now.ToString("hh:mm tt", CultureInfo.InvariantCulture);

                        string lugarFirmaMostrar = resultadoCVvehiculo.LugarFirma.ToString() ?? "";
                        var lugarfirma = await _buscarDistrito.buscarDistrito(int.Parse(lugarFirmaMostrar));
                        ...
                        string nombreNotario = ab != null ? $"{ab.Nombre} {ab.Apellido1} {(ab.Apellido2 ?? "")}".Trim() : (cedAbogado > 0 ? cedAbogado.ToString() : "");
                        string cedulaNotario = ab?.Cedula.ToString() ?? (cedAbogado > 0 ? cedAbogado.ToString() : "");

                        string nombreVendedor = prop != null ? $"{prop.Nombre} {prop.Apellido1} {(prop.Apellido2 ?? "")}".Trim() : (cedProp > 0 ? cedProp.ToString() : "");
                        string cedulaVendedor = prop?.Cedula.ToString() ?? (cedProp > 0 ? cedProp.ToString() : "");

                        string nombreComprador = comp != null ? $"{comp.Nombre} {comp.Apellido1} {(comp.Apellido2 ?? "")}".Trim() : (cedComp > 0 ? cedComp.ToString() : "");
                        string cedulaComprador = comp?.Cedula.ToString() ?? (cedComp > 0 ? cedComp.ToString() : "");

                        string marcaVehiculoNombre = idMarcaVeh.ToString();
                        string tipoVehiculoNombre = idTipoVeh.ToString();
                        string marcaMotorNombre = idMarcaMotor.ToString();
                        string combustibleNombre = idComb.ToString();
                        ...
                        string gastosRaw = (resultadoCVvehiculo.GastosTraspasoPagadosPor ?? "").Trim().ToUpperInvariant();
                        ...
                        html = html
                            ...
                            .Replace("{{ID_DOCUMENTO}}", resultadoCVvehiculo.IdDocumento.ToString())
                            .Replace("{{NUMERO_ESCRITURA}}", resultadoCVvehiculo.NumeroEscritura ?? "")
                            ...
                            .Replace("{{LUGAR_FIRMA}}", lugarfirma.NombreDistrito) 
                            .Replace("{{HORA_FIRMA}}", horaFirmaMostrar)
                            .Replace("{{FECHA_FIRMA}}", fechaFirmaMostrar);

                        var doc6 = new HtmlToPdfDocument
                        {
                        ...
                        var pdf6 = _converter.Convert(doc6);
                        return File(pdf6, "application/pdf");
                    }
                ...
                case "Poderes especiales judiciales":
                    ...
                    string fecha = resultadoPoderesesjudiciales.Fecha.ToString();
        ...
        {
            //var contexto = _context.THistorialDocumentos.Include(t => t.AbogadoNavigation).Include(t => t.ClienteNavigation);
            return View(await _listarHistorial.listar());
        }
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            /*var tHistorialDocumento = await _context.THistorialDocumentos
                .Include(t => t.AbogadoNavigation)
        ...
        {
            //ViewData["Abogado"] = new SelectList(_context.TGeAbogados, "Cedula", "Cedula");
            //ViewData["Cliente"] = new SelectList(_context.TGePersonas, "Cedula", "Apellido1");
    ...
    .ToList();
        ...
        // POST: THistorialDocumento1/Create
        // To protect from overposting attacks, enable the specific properties you want to bind to.
        // For more details, see http://go.microsoft.com/fwlink/?LinkId=317598.
        /*[HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create([Bind("Id,Fecha,Cliente,Abogado,TipoDocumento,IdDocumento,Titulo")] THistorialDocumento tHistorialDocumento)
        {
        ...
        // POST: THistorialDocumento1/Edit/5
        // To protect from overposting attacks, enable the specific properties you want to bind to.
        // For more details, see http://go.microsoft.com/fwlink/?LinkId=317598.
        /*[HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Fecha,Cliente,Abogado,TipoDocumento,IdDocumento,Titulo")] THistorialDocumento tHistorialDocumento)
        {
            if (id != tHistorialDocumento.Id)
            {
                return NotFound();
            }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    _context.Update(tHistorialDocumento);
                    await _context.SaveChangesAsync();
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!THistorialDocumentoExists(tHistorialDocumento.Id))
                    {
                        return NotFound();
                    }
                    else
        ...
        // GET: THistorialDocumento1/Delete/5
